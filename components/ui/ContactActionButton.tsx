...
export default function ContactActionButton({
  ...
  const disabledClass = disabled
    ? "cursor-not-allowed opacity-65 hover:translate-y-0 hover:shadow-none"
    : "";

  const commonClass = `inline-flex h-[42px] items-center gap-2 whitespace-nowrap rounded-[13px] border px-4 font-kanit text-[13px] font-medium transition-all duration-300 hover:-translate-y-0.5 max-[430px]:w-full max-[430px]:justify-center sm:h-[44px] sm:px-5 sm:text-[14px] lg:h-[46px] lg:text-[15px] ${variantClass} ${disabledClass} ${className}`.trim();
  ...
  return (
    <button type={type} className={commonClass} disabled={disabled}>
      {icon}
      {children}
    </button>
  );
